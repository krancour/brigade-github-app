...
const localPath = "/workspaces/brigade-github-app";
...
const buildGatewayJob = (e, p) => {
  return new MakeTargetJob(buildGatewayJobName, kanikoImg, e);
}
jobs[buildGatewayJobName] = buildGatewayJob;

const pushGatewayJobName = "push-gateway";
const pushGatewayJob = (e, p) => {
  return new PushImageJob(pushGatewayJobName, e, p);
}
...
function runSuite(e, p) {
  ...
  return Promise.all([
    ...
    // Docker images:
    run(e, p, buildGatewayJob(e, p)).catch((err) => { return err }),
    // Helm chart:
    ...
    if (e.revision.ref == "v2") {
      // Push "edge" images.
      Promise.all([
        run(e, p, pushGatewayJob(e, p)).catch((err) => { return err })
      ]).then((values) => {
        values.forEach((value) => {
          if (value instanceof Error) throw value;
        }); 
